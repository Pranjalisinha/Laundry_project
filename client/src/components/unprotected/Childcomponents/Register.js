...
const Register = (props) => {
  ...
  const listOfState= getStatesOfCountry("IN")
  const listOfCities=City.getCitiesOfCountry("IN")
...
<select   required id="State-register" onChange={(e)=>handleFormData(e,"State")}>
  <option value=""></option>
{ listOfState && listOfState.map((state,i)=>{
 return <option value={state.name} key={i+`${state.name}`}>{state.name}</option>
})}
</select>
<div className='blue-line-register'></div>
</div>
...
<option value=""></option>
{ listOfCities && listOfCities.map((city,i)=>{
 return <option value={city.name} key={i+`${city.name}`}>{city.name}</option>
})}
</select>
<div className='blue-line-register'></div>
</div>
